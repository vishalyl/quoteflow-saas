...
import { getCompanies, addCompany, updateCompany, deleteCompany } from '../db/queries.js'
...
export default function Companies() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal === 'add') {
        await addCompany(form.name.trim(), form.contact || null, form.notes || null)
        showToast('Company added!')
      } else {
        await updateCompany(editing.id, { name: form.name.trim(), contact: form.contact || null, notes: form.notes || null })
  ...
  const handleDelete = async id => {
    if (!window.confirm('Delete this company? All linked data will be unlinked.')) return
    try {
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">{uniqueCompanies.length} companies</div>
        </div>
        ...
        <table>
          ...
          <tbody>
            {uniqueCompanies.length === 0 ? (
              <tr><td colSpan={3} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>No companies yet</td></tr>
            ) : uniqueCompanies.map((c, i) => (
              <tr key={c.id}>
                <td style={{ color: 'var(--text-muted)' }}>{i + 1}</td>
                <td style={{ fontWeight: 500 }}>
                  <Link to={`/companies/${c.id}`} style={{ color: 'var(--primary-light)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 6 }}>
                    🏢 {c.name}
                  </Link>
                </td>
                <td>
        ...
        <div className="modal-backdrop" onClick={() => setModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3>{modal === 'add' ? 'Add Company' : 'Edit Company'}</h3>
              <button className="btn btn-ghost btn-icon" onClick={() => setModal(null)}>✕</button>
            </div>
            <div className="flex-col flex gap-12">
              <div className="form-group">
                <label>Company Name *</label>
                <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Company name..." autoFocus />
              </div>
              <div className="form-group">
              ...
              <div className="flex gap-8 justify-between">
                ...
                <div className="flex gap-8" style={{ marginLeft: 'auto' }}>
                  ...
                  <button className="btn btn-primary" onClick={handleSave} disabled={saving || !form.name.trim()}>
                    {saving ? 'Saving...' : modal === 'add' ? 'Add Company' : 'Save Changes'}
                  </button>
                </div>
              </div>
            </div>
