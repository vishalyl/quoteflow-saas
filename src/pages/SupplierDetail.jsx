...
import { getCompanies, getSupplierById, getSupplierDeals, getMasterProducts, getSuppliers, updateQuotation, updateQuotationItem, deleteQuotationItem } from '../db/queries.js'
import Papa from 'papaparse'

const fmt = n => n != null ? `₹${parseFloat(n).toLocaleString('en-IN', { minimumFractionDigits: 2 })}` : '—'

function flattenDeals(items) {
  return (items || []).map(item => ({
    id: item.id,
    quotation_id: item.quotations?.id,
    company_id: item.quotations?.company_id,
    date: item.quotations?.date || '', 
    status: item.quotations?.status || 'pending',
    raw_product_name: item.raw_product_name || '',
    master_name: item.master_products?.name || '—',
...
export default function SupplierDetail() {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [c, deals, mp, comps, supps] = await Promise.all([getSupplierById(id), getSupplierDeals(id), getMasterProducts(), getCompanies(), getSuppliers()])
      setSupplier(c)
  ...
  const onSave = async () => {
    ...
    try {
      ...
      await updateQuotation(f.quotation_id, { supplier_id: f.supplier_id || null, status: f.status, date: f.date })
      ...
      <div className="page-header">
        <div>
          <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 4 }}>
            <Link to="/suppliers" style={{ color: 'var(--primary-light)', textDecoration: 'none' }}>← Suppliers</Link>
          </div>
        ...
        <button className="btn btn-secondary" onClick={() => {
          ...
          const rows = filtered.map(row => ({
            ...
            'Company': companies.find(c => c.id === row.company_id)?.name || '',
          ...
          a.download = `${(supplier?.name || 'supplier').replace(/[^a-z0-9]/gi, '_')}_deals_${new Date().toISOString().slice(0, 10)}.csv`; a.click()
          URL.revokeObjectURL(url)
        }}>⬇ Export CSV</button>
      </div>

      {/* Filters — same style as Master Database, no search, no supplier */}
      ...
      <div className="card mb-20">
        <div className="table-wrapper">
          <table className="editable-table">
            <thead>
              <tr><th>Date</th><th>Status</th><th>Product</th><th>Qty</th><th>Unit</th><th>Cost</th><th>Quoted</th><th>Margin</th><th>Company</th><th></th></tr>
            </thead>
            <tbody>
              {visibleRows.map(row => (
                <tr key={row.id}>
                  ...
                  <td>{row.quotations?.companies?.name || '—'}</td>
                  <td>
          ...
          <div className="modal" style={{ maxWidth: 500 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'minmax(120px, 2fr) 1fr 1fr', gap: 12 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <label className="label-sm">Company (Customer)</label>
